<template>
  ...
  <div class="buttonBox">
    ...
    <button @click="send(this.db.getVersion)">sg.getVersion</button>
    <button @click="send(this.db.init)">sg.init</button>
    <button @click="send(this.db.askAndAnswer)">sg.askAndAnswer</button>
    <button @click="send(this.db.listen)">sg.listen</button>
    <button @click="send(this.db.close)">sg.close</button>
    <button @click="clear">清空日志</button>
  </div>
  <div class="pcConfig">
    平台:<input id="inPlatform" value="auto" />
    pcIp:<input id="inIP" value="localhost" />
    port:<input id="inPort" value="8888" />
  </div>
